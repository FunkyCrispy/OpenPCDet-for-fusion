...
## Installation (the same as original OpenPCDet)
```
conda create -n openpcdet (optional)
conda activate openpcdet
pip install numpy=1.17 (must be <= 1.20)
pip install torch==1.7.1+cu110 torchvision==0.8.2+cu110 torchaudio==0.7.2 -f https://download.pytorch.org/whl/torch_stable.html
pip install -r requirements.txt
pip install spconv-cu111 (for other CUDA version, using other commands: https://github.com/traveller59/spconv)
pip install vtk
pip install pyqt5-tools
pip install mayavi
python setup.py develop (This step compile and generate some .so files)
```
If error **'qt.qpa.plugin: Could not load the Qt platform plugin "xcb" in ...'** occurs:
```
...
Some modifications are made to OpenPCDet project to train with semantic point cloud data.

Download KITTI dataset first, and **modify the data path and save data path of script below**. Then generate semantic point cloud data by running script:
```
python produce_sem_velydyne_data.py 
```
This script will create another folder to put generated .bin files (total volume larger than KITTI velodyne part).
Only training data part are processed, because KITTI testing data do not have label files, which is required for semantic information.

**'Original point cloud data has 4 dimensions'**: (x, y, z, intensity)
**'Current point cloud data has 8 dimensions'**: (x, y, z, intensity, **one-hot**), one-hot is for 4 classes:(background, car, pedestrain, cyclist), each point is transformed through matrix from lidar to camera. Each sample has a corresponding calib.txt, which contains 2D bboxes of each object. If the transformed point locate inside a 2D bbox, then the bbox's class information is known and easy to produce one-hot vector.

Then put generated folder **'data_object_velodyne_sem/training'** in project path **'./data/kitti'**, so as to replace original KITTI velodyne data. Run command below to generate .pkl file:
```
python -m pcdet.datasets.kitti.kitti_dataset create_kitti_infos tools/cfgs/dataset_configs/kitti_dataset.yaml
```
Notice that you have to modify the ./pcdet/datasets/kitti/kitti_dataset.py  **'get_lidar'** function, change line:
```
return np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 4)  --  to:
return np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 8)
```
Because in .bin files, the points stored as a 1 dimension array, so reshape is needed. And current dimension of point is 8 rather than 4.

Then modify files to correctly training model. **'./tools/cfgs/dataset_configs/kitti_dataset.yaml'**:
```
DATA_PATH (if you change data folder)
DATA_AUGMENTOR/AUG_CONFIG_LIST[0]/NUM_POINT_FEATURES: 4 --> 8
DATA_AUGMENTOR/AUG_CONFIG_LIST[0]/USE_ROAD_PLANE: True --> False  (so that you don't have to download road relative KITTI dataset)
POINT_FEATURE_ENCODING: (no need to be the same, but have to have another 4 names for 4 new dimensions)
  used_feature_list: ['x', 'y', 'z', 'intensity', 'onehot_bg', 'onehot_car', 'onehot_pedestrian', 'onehot_cyclist'],
  src_feature_list: ['x', 'y', 'z', 'intensity', 'onehot_bg', 'onehot_car', 'onehot_pedestrian', 'onehot_cyclist'],
```
Take the PointPillar model config as a example, modification to **'./tools/cfgs/kitti_models/pointpillar.yaml'**:
```
DATA_AUGMENTOR/AUG_CONFIG_LIST[0]/USE_ROAD_PLANE: True --> False
DATA_AUGMENTOR/AUG_CONFIG_LIST[0]/NUM_POINT_FEATURES: 4 --> 8
```

Finally, you can run **'./tools/train.py'** to train your model. (You can also directly use train_pointpillar.py. For convenience, I copy train.py kitti_dataset.yaml, pointpillar.yaml, adding suffix **'_fc'**, and change paths value to read them correctly.)
...
# OpenPCDet (original ReadME part)
